...
export function buildRequest(args: string[], opts: ArgOpts): Request {
  ...
  const explicitMethod = args[0]?.toUpperCase() === method
  const requestArgs = explicitMethod ? args.slice(1) : [...args]
  ...
  init.method = method
  if (!explicitMethod && method === "GET" && init.body) init.method = "POST"
