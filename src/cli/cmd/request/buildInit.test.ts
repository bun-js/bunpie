...
test("buildInit routes query params, headers, and body fields", () => {
  ...
  const { url: requestUrl, init } = buildInit(["a==1", "x:1", "name=bun", "file:=blob", "skip=="], url, jsonOpts)
...
test("buildInit parses command-line body fields using json semantics", () => {
  const url = new URL("http://example.com/")
  const { init } = buildInit(["name=bun", "count:=42", "active:=true", "disabled:=false", "ratio:=3.14", "empty:=null", "items:=[1,2,3]", 'meta:={"version":1,"ok":false}'], url, jsonOpts)
  expect(init.body).toBe(JSON.stringify({ name: "bun", count: 42, active: true, disabled: false, ratio: 3.14, empty: null, items: [1, 2, 3], meta: { version: 1, ok: false } }))
})
...
test("buildInit preserves delimiter characters inside values", () => {
  const url = new URL("http://example.com/")
  const { url: requestUrl, init } = buildInit(["q==a==b", "auth:Bearer x:y", "target=https://example.test/a:b?x=1&y=2", 'raw:={"url":"https://example.test/a:b?x=1&y=2"}'], url, jsonOpts)
  expect(url.searchParams.get("q")).toBeNull()
  expect(requestUrl.searchParams.get("q")).toBe("a==b")
  expect(headersOf(init).get("auth")).toBe("Bearer x:y")
  expect(init.body).toBe(JSON.stringify({ target: "https://example.test/a:b?x=1&y=2", raw: { url: "https://example.test/a:b?x=1&y=2" } }))
})
...
test("buildInit uses form encoding when requested", () => {
  const url = new URL("http://example.com/")
  const { init } = buildInit(["name=bun"], url, { ...jsonOpts, form: true, json: false })
...
test("buildInit keeps json-style form fields as raw form values", () => {
  const url = new URL("http://example.com/")
  const { init } = buildInit(["count:=42", "items:=[1,2]"], url, { ...jsonOpts, form: true, json: false })
  expect(init.body?.toString()).toBe("count=42&items=%5B1%2C2%5D")
})

test("buildInit does not override user-provided default headers", () => {
  const url = new URL("http://example.com/")
  const { init } = buildInit(["content-type:text/plain", "accept:text/html", "name=bun"], url, jsonOpts)
...
test("buildInit does not override user-provided form content type", () => {
  const url = new URL("http://example.com/")
  const { init } = buildInit(["content-type:multipart/form-data", "name=bun"], url, { ...jsonOpts, form: true, json: false })
